...
Status after the Haskell 98 replacement: **resolved**. All 22 complete modules
below were checked with the current CLI. Each terminates with `KindInfinite`
at the self-application in its declaration. The old settling engine is gone.

The observations, mechanism and controls below describe the removed engine.
They are retained as investigation evidence, not current expected behavior.
Current runnable successes and failures are listed in [README.md](README.md).

## Historical observation

Each block is a complete `src/Main.nash` for an application project
(`{"type":"application","dependencies":{}}` in `nash.jsonc`).

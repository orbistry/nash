    ...
    match &src_type.value {
        ...
        CanType::Alias {
            ...
            let body = match target {
                CanAliasType::Open(body) | CanAliasType::Filled { body, .. } => *body,
            };
            if !remaining.is_empty() {
                assert!(
                    matches!(target, CanAliasType::Open(_)),
                    "partial alias body must be closed"
                );
                return bump.alloc(Type::PartialAliasN {
                    home: reference.home,
                    name: reference.name,
                    args: targs,
                    remaining,
            ...
            let real = match target {
                CanAliasType::Filled { typ: real_type, .. } => {
                    from_src_type(bump, free_vars, real_type)
                }

...
fn assemble_selected_ledger_language() {
    ...
    for version in [PlutusVersion::V1, PlutusVersion::V2, PlutusVersion::V3] {
        ...
        assert_eq!(compiled.program.version.is_v1_1_0(), version == PlutusVersion::V3);
        assert_eq!(pretty::term(compiled.program.eval_version(&a,version).term.unwrap()), "(con integer 42)");
    }
    let constr = b.constr(0, &[b.int(1)]);
    assert!(matches!(assemble_core_for_version(&a,constr,PlutusVersion::V1),Err(Error::Target(_))));
    assert!(matches!(assemble_core_for_version(&a,constr,PlutusVersion::V2),Err(Error::Target(_))));
    ...
    assert!(matches!(assemble_core_for_version(&a,newer,PlutusVersion::V3),Err(Error::Target(_))));
}

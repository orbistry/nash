...
- [ ] 01 Syntax: `'a` type vars, little/Big names, `trait`/`impl`, `=>` contexts, kind annotations, `validator module`, `tests` block, `do`, attributes, `name!()`, `comptime`, `assert`/`fail`/`todo`/`trace`; drop `Float`/`Char`/record extension types — nested-section verification fix pending; see [plans/01-syntax.md](plans/01-syntax.md)

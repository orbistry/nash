...
| Named ctor fields | `type Datum = Datum { owner : Bytes, deadline : Int }` is a constructor with *labeled fields* (Aiken style), not a nested record: encoded flat (`Constr 0 [B, I]` / `constr 0 [..]`), fields follow the enclosing type's representation rule, `.field` access allowed on single-constructor types, record-style construction `Datum { owner = o, deadline = d }` and pattern `Datum { owner, deadline }` allowed; record updates on alias records and single-constructor labeled types. Anonymous record *types* are not allowed anywhere else; use `type alias`. |
| Prelude twins | `bool`/`Bool`, `unit`/`Unit`, `option`/`Option`, `ordering`/`Ordering`. Little constructors are exposed unqualified by the prelude (`True`, `Some`, `LT`...); Big twins only qualified (`Bool.True`, `Option.Some`). `if` takes `bool`. |
| `Data` | Big type with constructors `Constr payload | Map kvs | List xs | I n | B bs`; pattern-matchable. Independent Validate and Decode traits perform source-level checking. |
| Big <-> little | Explicit. `ToData`/`FromData` on any type, with explicit little conversions and Big-only blanket implementations (`toData` implemented with `Primitive.coerce` through an ordinary blanket impl for all Big types, unchecked `fromData` implemented with `Primitive.coerce` through its own blanket impl, and opt-in `Validate.validate` for recursive checking in Nash source). `Lift 'small 'big` multi-param trait (`lift`/`lower`) between reprs; built-in reflexive `impl Lift 'a 'a` for every type (exempt from head rules). Lift/lower preserve container payloads and change only the outer representation; `Lift (list ('a : Big)) (List 'a)` wraps directly without traversal. `validate : Data -> 'a` traps on mismatch; `Data.decode` is the non-failing path. `Primitive.coerce : 'a -> 'b` is explicit unchecked runtime identity for any value types, including functions; it changes no representation and validates no shape. No implicit coercion. |
| Literals | Polymorphic via `FromInt` / `FromString` / `FromBytes` / `FromBool` / `FromUnit` traits. Ambiguous literals default to little (`int`, `string`, `bytes`, `bool`, `unit`). |
...
| Property testing | Little `Choice`/`Group` trees with strict group replay; `Prop.generator` is a state-function alias with ordinary Functor/Applicative/Monad composition. Rust reduces traces using consumed primitive-choice shortlex. Each prop prepares its state, body function, and display function once. |

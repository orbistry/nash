...
| Property testing | `type prng = Seeded bytes (list int) \| Replayed int (list int)` (little, built by the runner as native terms), choice-sequence shrinking in Rust, `generator 'a` little type with Functor/Applicative/Monad. Each prop compiles to `draw`/`run` programs. |

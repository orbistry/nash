...
| Big <-> little | Explicit. `ToData`/`FromData` on Big types (`toData` defaulting to `Builtin.coerce` through an ordinary blanket impl for all Big types, unchecked `fromData` defaulting to `Builtin.coerce` through its own blanket impl, and opt-in `Validate.validate` for recursive checking in Nash source). `Lift 'small 'big` multi-param trait (`lift`/`lower`) between reprs; built-in reflexive `impl Big 'a => Lift 'a 'a` (exempt from head rules) so `Lift 'a 'b => Lift (list 'a) (List 'b)` covers `list Int`. `validate : Data -> 'a` traps on mismatch; `Data.Decode` is the non-failing path. `Builtin.coerce : 'a -> 'b` is explicit unchecked runtime identity for any value types, including functions; it changes no representation and validates no shape. No implicit coercion. |

...
| Property testing | `type prng = Seeded bytes (list int) \| Replayed (list int)` (little, built by the runner as native terms), choice-sequence shrinking in Rust, `generator 'a` function alias with Functor/Applicative/Monad. Each prop prepares its state, body function, and display function once. |
